...
* **Cite Prior Tool Calls First:** Before running anything new, check whether the proof already exists in this conversation. If an earlier `Read`, `Grep`, `Glob`, or `Bash` output already substantiates the claim, quote it directly — e.g. *"In the `Read` of `foo.ts` earlier this turn, line 23 shows `…`."* This is the cheapest, fastest proof and avoids re-running commands the user has already seen.
* **Prioritize Empirical Proof:**
    * If it's **code**, run it (or a test case of it) to verify the output.
    * If it's a **file or codebase**, use tools to read, grep, or list the actual content.
    * If it's an **external fact**, perform a fresh search and provide the specific source or snippet.
* **Guard Against Destructive Proofs:** Never run a verification that has real-world side effects (sending email, charging payments, writing to production, posting to Slack/GitHub, dropping data, force-pushing, etc.) without confirming with the user first. Prefer a dry-run, a unit test against a fake, a read-only inspection of the code path, or simply *describing* what would happen. If only a side-effectful proof will satisfy the user, ask explicitly before proceeding.
